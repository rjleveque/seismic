...
c
 20   if (ncycle .ge. nstop .or. time .ge. tfinal) goto 999
      ...
      if (nextevent .le. nevents) then
        eventtime = event_times(nextevent)
      else
        eventtime = rinfinity
      endif
...
c
      ...
      if (time.lt.eventtime .and. time + possk(1) .ge. eventtime) then
c       ## adjust time step to hit event time exactly
        possk(1) = eventtime - time
        do 14 i = 2, mxnest
 14       possk(i) = possk(i-1) / kratio(i-1)
