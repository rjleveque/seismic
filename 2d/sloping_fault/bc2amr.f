...
c     # extend values across the surface:
      do 405 m=1,meqn
         do 405 j=jbeg,ncol
            do 405 i=1,nrow
...
c     # adjust the stress:
      do 406 j=jbeg,ncol
         do 406 i=1,nrow
           dnormx = aux(9,i,jbeg)
           dnormy = aux(10,i,jbeg)
           tempsigxy = val(3,i,j)*(dnormx*dnormx - dnormy*dnormy)
     &         - 2.d0*dnormx*dnormy*val(1,i,j)
           val(2,i,j) = -val(2,i,j) - dnormx/dnormy*(
     &                  val(3,i,j) + tempsigxy)
           val(1,i,j) = val(1,i,j) - dnormx/dnormy*(
     &                  val(3,i,j) - tempsigxy)
           val(3,i,j) = tempsigxy
  406    continue
      go to 499
